...
namespace octomap_wrapper{
   ...
   */
  static inline octomap::AbstractOcTree* fullMsgToMap(const OctomapWrapper& wrapper){
    octomap::AbstractOcTree* tree = octomap::AbstractOcTree::createTree(wrapper.id, wrapper.resolution);
    if (tree){
   ...
   */
  template <class OctomapT>
  static inline bool binaryMapToMsg(const OctomapT& octomap, OctomapWrapper& wrapper){
    ...
    std::string datastring = datastream.str();
    wrapper.data = std::vector<int8_t>(datastring.begin(), datastring.end());
    return true;
  }
   ...
   */
  template <class OctomapT>
  static inline bool fullMapToMsg(const OctomapT& octomap, OctomapWrapper& wrapper){
	wrapper.resolution = octomap.getResolution();
	wrapper.id = octomap.getTreeType();
